...
class RubiksCubeVisualizer:
    ...
    def rotate_face(self, f, n=1, layer=0):
        ...
        if (f == f_last) and (layer == layer_last):
            # Combine the rotations: add the current rotation to the last one and take modulo 4
            ntot = (n_last + n) % 4
            # If rotating the opposite way is shorter, adjust the total rotation
            if abs(ntot - 4) < abs(ntot):
                ntot = ntot - 4
            # If the combined rotation results in no movement, remove the last move from the list
