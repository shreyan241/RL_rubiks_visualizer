...
class Quaternion:
    ...
    def from_v_theta(cls, v, theta):
        """
        Construct quaternions from unit vector v and rotation angle theta.
        Input can be a batch of unit vectors and corresponding rotation angles.
    ...
    def as_v_theta(self):
        ...
        """
        q = self.q.reshape((-1, 4)).T

        # compute theta
        norm = np.sqrt((q ** 2).sum(0))
        theta = 2 * np.arccos(q[0] / norm)

        # compute the unit vector
        v = np.array(q[1:], order='F', copy=True)
        v /= np.sqrt(np.sum(v ** 2, 0))

        # reshape the results
        v = v.T.reshape(self.q.shape[:-1] + (3,))
        theta = theta.reshape(self.q.shape[:-1])
    ...
    def as_rotation_matrix(self):
        ...
        Input can be a batch of quaternions.
        """
...
def project_points(points, q, view, vertical=[0, 1, 0]):
    ...
    if np.all(xdir == 0):
        raise ValueError("vertical is parallel to view")
